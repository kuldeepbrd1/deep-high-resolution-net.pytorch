...
#For Testing one can selectively take images for testing (useful for heatmap generation especially wiwth Volab, where drive-colab links are shitty)
    ...
    def _get_ann_file_keypoint(self):
        ...
        prefix = self.image_set #'train' \
            #if cfg.DATASET.TEST_SET not in self.image_set else cfg.DATASET.TEST_SET
        return os.path.join(
            self.root,
            prefix  + '.json'
        )
        ...
        for obj in objs:
            ...
            if obj['area'] > 0 and x2 >= x1 and y2 >= y1:
              if self.make_selection:
                if obj['image_id']>=self.resume_id and obj['image_id']<self.stop_id:
    ...
    def evaluate(self, cfg, preds, output_dir, all_boxes, img_path,
        ...
        if 'val' not in self.image_set and 'test' not in self.image_set:
