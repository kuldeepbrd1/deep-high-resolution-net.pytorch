...
class COCO_SatPose:
    ...
    def __init__(self, **kwargs):
        ...
        self.dataset_objects = {"SPEED":"Tango", "ENVISAT-1": "Envisat"}
        self.object = self.dataset_objects[self.dataset]
        kwargs["object"] = self.object

        self.keys= ["info","licenses","images","annotations","categories","segment_info"]
        self.info = info().getDatasetInfo(self.dataset)
        self.categories=[Category(**kwargs).get_dict()]
        kwargs["category"] = self.getCategoryinfo()


        self.images,self.annotations = Images(**kwargs).get_images_annotations()
        self.num_examples = len(self.images)
        self.licenses=self.get_licenses()
        self.json_dict = self.compose()

    def compose(self):
        return {"info" : self.info, "licenses": self.licenses, "images": self.images, "annotations": self.annotations, "categories":self.categories}
...
class Licenses:
    ...
    ---------------------------------------------------'---'''
    def __init__(self):
        self.dataset_license = {"SPEED":"cc-by-nc-sa-3.0", "ENVISAT-1":"None"}
        self.License_dict = self.supportedLicenses()

    def supportedLicenses(self):
        by_na_sa3 ={"id": 1, "url":"https://creativecommons.org/licenses/by-nc-sa/3.0/", "name": "Attribution-NonCommercial-ShareAlike 3.0"}
        no_license ={"id": 0, "url":"N/A", "name": "N/A"}

        Licenses = {"None": no_license, "cc-by-nc-sa-3.0":by_na_sa3}
        return Licenses
...
class Category:
    ...
    def __init__(self,**kwargs):
        ...
        self.object = kwargs["object"] if kwargs["object"] in list(self.objects) else None
        self.id = self.objects[self.object]
        self.keypoints = [str(i) for i in range(1,kwargs['n_keypoints']+1)]
        self.skeleton = kwargs['skeleton']

    '''   Deprecated. Bad code to add constants here. Should be passed through kwargs for visibility
    def get_all_KP_data(self):
        #Add more KP/skeleton models here
        tango_KPs = [str(i) for i in range(1,12)]
        tango_skeleton = [[1,2],[2,3],[3,4],[4,1],[1,5],[2,6],[3,7],[4,8],[5,6],[6,7],[7,8],[8,5],[1,9],[2,10],[4,11]]
        all_dict = {"Tango":{"keypoints":tango_KPs,"skeleton":tango_skeleton}}

        return all_dict

    def get_KPs_skeleton(self,all_KP_data, obj_name):
        return all_KP_data[obj_name]["keypoints"], all_KP_data[obj_name]["skeleton"]
    '''
    def get_dict(self):
        category= {"supercategory":self.supercategory,"id":self.id, "name":self.object, "keypoints": self.keypoints, "skeleton":self.skeleton}
        return category
...
class Images :
    ...
    def __init__(self, **kwargs):
        ...
        self.data,self.index_map  = self.getDatafromFile(kwargs["datafile"])
        self.img_dir = kwargs["image_path"]
        self.annotations, self.ImageList = self.processImages(**kwargs) #Self.List is the dict object


    def getCocoDict(self,filename,ID):
        image =  {"license":self.license, "file_name":filename, "coco_url":"" , "height":self.height, "width": self.width, "date_captured": "", "url":self.url, "id": ID}
        return image
    ...
    def processImages(self,**kwargs):
        ...
        self.image_path_list = glob.glob(os.path.join(img_path,"*.jp*g"))
        image_obj_list = []
        annotations = []
        ...
        for idx,path in enumerate(self.image_path_list):
            ...
            img_id = int((img_name.split('.')[0]).strip(string.ascii_letters))
            img_dict = self.getCocoDict(img_name,img_id)
            img_data = self.data[self.index_map[img_name]]
            annotation = Annotation(idx+1, img_id,img_name, img_data, **kwargs)
...
class Annotation:
    ...
    def __init__(self, annotation_id, ID, name ,data, **kwargs):
    ...
    def getImgInfofromData(self):
        if self.data["filename"] == self.name:
            bbox = self.data["label"] if 'label' in list(self.data) else self.data["bbox"]  #bbox key should be label or bbox #changed for envisat
            bbox, area = self.bbox_XY2WH(bbox)
            features = self.data["features"]
            kps_coco = []
            for feature in features:
                coordinates = list(feature["Coordinates"])
                visibility = 2 if feature["Visibility"]==1 else 0
    ...
    def getAnnotationDict(self):
        annotation = {"segmentation": self.segmentation, "num_keypoints":self.num_keypoints, "area":self.area, "iscrowd": self.iscrowd, "keypoints":self.keypoints, "image_id":self.image_id, "bbox": self.bbox, "category_id":self.category_id, "id":self.id}
        return annotation
